...
export async function toggleSuperAdminApi(userId: string, isAdmin: boolean): Promise<unknown> {
...
export async function fetchUserMembershipsApi(userId: string): Promise<Membership[]> {
  return apiRequest<{ memberships: Membership[] }>(
    `/api/v1/admin/users/${userId}/memberships`
  ).then((data) => data.memberships);
}

export async function addUserToOrgApi(
  userId: string,
  organizationId: string,
  role: 'admin' | 'staff'
): Promise<unknown> {
...
export async function removeUserFromOrgApi(userId: string, membershipId: string): Promise<unknown> {
  return apiRequest(`/api/v1/admin/users/${userId}/memberships/${membershipId}`, {
    method: 'DELETE',
  });
}
...
export async function fetchAdminOrganizationsList(): Promise<AdminOrganization[]> {
  return apiRequest<{ organizations: AdminOrganization[] }>('/api/v1/admin/organizations').then(
...
export function* fetchAdminUsersWorker(action: PayloadAction<FetchAdminUsersPayload>) {
...
export function* fetchAdminUserWorker(action: PayloadAction<FetchAdminUserPayload>) {
...
export function* toggleSuperAdminWorker(action: PayloadAction<ToggleSuperAdminPayload>) {
  ...
  try {
    ...
    toast.success(isAdmin ? 'Super Admin access granted' : 'Super Admin access revoked');
...
export function* fetchUserMembershipsWorker(action: PayloadAction<FetchUserMembershipsPayload>) {
  const { userId } = action.payload;
  try {
...
export function* addUserToOrgWorker(action: PayloadAction<AddUserToOrgPayload>) {
...
export function* updateAdminMemberRoleWorker(action: PayloadAction<UpdateAdminMemberRolePayload>) {
...
export function* removeUserFromOrgWorker(action: PayloadAction<RemoveUserFromOrgPayload>) {
...
export function* fetchAdminOrganizationsWorker() {
  try {
