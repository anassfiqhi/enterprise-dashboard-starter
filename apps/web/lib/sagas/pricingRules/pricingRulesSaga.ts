...
export async function fetchPricingRulesApi(hotelId: string): Promise<PricingRule[]> {
  return apiRequest<PricingRule[]>(`/api/v1/hotels/${hotelId}/pricing-rules`);
}

export async function createPricingRuleApi(
  hotelId: string,
  body: Omit<PricingRule, 'hotelId'>
): Promise<PricingRule> {
  return apiRequest<PricingRule>(`/api/v1/hotels/${hotelId}/pricing-rules`, {
...
export async function deletePricingRuleApi(hotelId: string, id: string): Promise<void> {
  return apiRequest<void>(`/api/v1/hotels/${hotelId}/pricing-rules/${id}`, {
    method: 'DELETE',
  });
}

export function* fetchPricingRulesWorker(action: PayloadAction<FetchPricingRulesPayload>) {
  try {
...
export function* createPricingRuleWorker(action: PayloadAction<CreatePricingRulePayload>) {
...
export function* updatePricingRuleWorker(action: PayloadAction<UpdatePricingRulePayload>) {
...
export function* deletePricingRuleWorker(action: PayloadAction<DeletePricingRulePayload>) {
