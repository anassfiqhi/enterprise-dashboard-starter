...
import { useInvitations, useInviteMember, useCancelInvitation } from '@/hooks/useInvitations';
...
function MembersTab() {
  ...
  if (error) {
    return (
      <p className="text-destructive py-8 text-center">Failed to load members: {error.message}</p>
    );
  }

  const handleRemoveMember = (memberId: string, memberIdOrEmail: string) => {
    if (pendingRemove === memberId) {
      removeMember.mutateAsync({ memberIdOrEmail }).finally(() => setPendingRemove(null));
    } else {
  ...
  if (!members || members.length === 0) {
    return <p className="text-muted-foreground py-8 text-center">No members found</p>;
  }
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead>Email</TableHead>
          <TableHead>Role</TableHead>
          {canDeleteMembers && <TableHead className="w-[100px]">Actions</TableHead>}
        </TableRow>
      </TableHeader>
      <TableBody>
        {members.map((member) => {
            ...
            <TableRow key={member.id}>
              ...
              {canDeleteMembers && (
                <TableCell>
                  {!isCurrentUser && (
                    <Button
                      variant={pendingRemove === member.id ? 'destructive' : 'ghost'}
                      size="sm"
                      onClick={() => handleRemoveMember(member.id, member.id)}
                      disabled={removeMember.isPending}
                    >
                      {pendingRemove === member.id ? 'Confirm?' : <Trash2 className="h-4 w-4" />}
                    </Button>
                  )}
                </TableCell>
              )}
...
function InvitationsTab() {
  ...
  const handleCancel = (invitationId: string) => {
    if (pendingCancel === invitationId) {
      cancelInvitation.mutateAsync({ invitationId }).finally(() => setPendingCancel(null));
    } else {
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleInvite} className="flex gap-4">
              ...
              <Select
                value={role}
                onValueChange={(v) => setRole(v as 'staff' | 'manager' | 'admin' | 'owner')}
              >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {canCancel && <TableHead className="w-[100px]">Actions</TableHead>}
                </TableRow>
              </TableHeader>
              <TableBody>
                {pendingInvitations.map((invitation) => (
                  <TableRow key={invitation.id}>
                    ...
                    {canCancel && (
                      <TableCell>
                        <Button
                          variant={pendingCancel === invitation.id ? 'destructive' : 'ghost'}
                          size="sm"
                          onClick={() => handleCancel(invitation.id)}
                          disabled={cancelInvitation.isPending}
                        >
                          {pendingCancel === invitation.id ? 'Confirm?' : <X className="h-4 w-4" />}
                        </Button>
                      </TableCell>
                    )}
                  </TableRow>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Manage members and invitations for your organization</CardDescription>
      </CardHeader>
      <CardContent>
